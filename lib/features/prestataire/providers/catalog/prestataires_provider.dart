...
final prestatairesFilteredProvider =
    Provider<AsyncValue<List<PrestataireCatalogEntry>>>((ref) {
      ...
      final likedIds = ref.watch(clientLikedPrestataireIdsProvider).maybeWhen(
            data: (ids) => ids,
            orElse: () => const <String>{},
          );
      return asyncList.whenData((entries) {
        ...
        if (filters.favoritesOnly) {
          list = list
              .where((e) => favoriteIds.contains(e.profile.id))
              .toList();
        }
        if (filters.likedOnly) {
          list = list.where((e) => likedIds.contains(e.profile.id)).toList();
        }
        return list;
      });
    });
