...
class _PrestataireStripeConnectTileState
  ...
  Future<bool> _launchStripeUrl(String url) async {
  ...
  Future<void> _openOnboarding() async {
    ...
    try {
      ...
      if (url == null || url.isEmpty) {
        _snack(
          'Stripe n’a pas renvoyé de lien d’inscription. Réessaie ou contacte le support.',
          error: true,
        );
        return;
      }

      if (!await _launchStripeUrl(url)) {
        _snack(
          'Impossible d’ouvrir le navigateur. Vérifie qu’un navigateur est installé, '
          'puis réessaie.',
          error: true,
        );
      }
