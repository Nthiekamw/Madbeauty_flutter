...
/// True sur Android/iOS/Web, avec un [DefaultFirebaseOptions] non placeholder.
///
...
bool isFirebaseConfiguredForPush() {
  if (kIsWeb) {
    return _isFirebaseWebPushConfigured();
  }
  ...
  return _hasValidFirebaseOptions(DefaultFirebaseOptions.currentPlatform);
}

bool _isFirebaseWebPushConfigured() {
  if (AppConfig.firebaseWebVapidKey.trim().isEmpty) return false;
  return _hasValidFirebaseOptions(DefaultFirebaseOptions.web);
}

bool _hasValidFirebaseOptions(FirebaseOptions options) {
  try {
    if (options.projectId.isEmpty || options.apiKey.isEmpty) return false;
    const placeholders = {'madbeauty-placeholder'};
    if (placeholders.contains(options.projectId)) return false;
